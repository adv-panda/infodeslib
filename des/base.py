...
class BaseDES: 
    ...
    def get_local_feature_importance(self, query):
        n_models = len(self.selected_models_indices) 
        feature_importance_dfs = [] 
        labels = [] 
        
        for i in self.selected_models_indices: 
            label = self.pool_classifiers[i].__class__.__name__ + " - W [" + str(self.selected_models_weights[i]) + "]"  
            shap_values = self.explainer_list[i].shap_values(query[self.feature_subsets[i]]) 
            
            df = pd.DataFrame() 
            df['Features']    = self.feature_subsets[i]
        ...
        n_cols = math.ceil(n_models/2)
        fig, axes = plt.subplots(n_cols, 2, sharex=True, figsize=(16,8)) 
        fig.suptitle('The local feature importance') 
            
        x = 1  
        y = 1 
        for j in range(n_models): 
            print(feature_importance_dfs[j][:10].Features)
            if n_cols == 1:                 
                sns.barplot(ax=axes[x-1], data=feature_importance_dfs[j][:10], y="Features", x='Shap_values', color='Orange') 
                axes[x-1].set_title(labels[j])
            else: 
                sns.barplot(ax=axes[y-1, x-1], data=feature_importance_dfs[j][:10], y="Features", x='Shap_values', color='Orange')      
                axes[y-1, x-1].set_title(labels[j]) 
            if x%2 == 0: 
                x  = 1 
                y += 1 
            else: 
